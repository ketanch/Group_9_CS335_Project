...
from code_gen import gen_var_offset, generate_final_code
...
class CParser:
    ...
    def p_postfix_expression_5(self, p):
        ...
        if var_data==None:
            ...
            size=var_data["type"]
        # try:
        #     t=get_var_type(p[3],global_stack,global_node)
        #     if(t!='int'):

        # self.total_errors += 1        
        # pr_error("Only int allowed in array access at line no. %d"%(p.lineno(1)))
        # except:
        #     pass
        ...
        try:
            ...
            if (index<0 or index>=size) and not size==0:
    ...
    def p_function_definition_init(self, p):
        ...
        try:
            ...
            add_arguments(p,p[2].children[1],global_node,global_stack,p[2].children[0].idName,1)
        except:
    ...
    def parse_inp(self, input):
        ...
        if self.total_errors != 0:
            self.total_errors += 1
            pr_error("Errors in compilation. Terminating!")
            exit(0)
